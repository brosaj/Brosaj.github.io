...
$(window).scroll(function () {
  ...
  var $window = $(window),
      // $bodyContainer = $('body'),
  $body = $('.noise'),
      $panel = $('.section--container'); // Change 33% earlier than scroll position so colour is there when you arrive.
  ...
  $panel.each(function () {
    ...
    if ($this.position().top <= scroll && $this.position().top + $this.height() > scroll) {
      ...
      $body.addClass('color-' + $(this).data('color')); // $bodyContainer.addClass('scroll');
    }
  });
}).scroll();